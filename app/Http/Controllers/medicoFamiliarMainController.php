...
class medicoFamiliarMainController extends Controller
{
    //Retornar la vista de inico del médico de medicina familiar
    public function index() {
        $userName = Session::get('nombreUsuario');
        $userImage = Session::get('fotoUsuario');
        
        if($userName != "") {
            return view('medicoFamiliar.inicio', ['nombreUsuario' => $userName, 'userImage' => $userImage]);
        }
        else {
            return redirect('/');
        }
    ...
    public function cambiarPasswordForm(){
        ...
        if($userName != "") {
            return view('medicoFamiliar.cambiarPassword', ['nombreUsuario' => $userName, 'error' => '',  'userImage' => $userImage]);
        }
        else {
            return redirect('/');
        } 
    ...
    public function cambiarPasswordPost(Request $request){
        ...
        if($userPassword[0]->password == $request->oldPassword) {
            ...
            return view('medicoFamiliar.cambiarPassword', ['nombreUsuario' => $userName, 'error' => 'Error: La contraseña actual es incorrecta.', 'userImage' => $userImage]);
        }
