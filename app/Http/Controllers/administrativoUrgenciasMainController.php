...
class administrativoUrgenciasMainController extends Controller
{
    //Retornar la vista de inico del administrativo de urgencias
    public function index() {
        ...
        if($userName != "") {
            return view('administrativoUrgencias.inicio', ['nombreUsuario' => $userName, 'userImage' => $userImage]);
        }
        else {
            return redirect('/');
        }  
    ...
    public function cambiarPasswordForm(){
        ...
        if($userName != "") {
            return view('administrativoUrgencias.cambiarPassword', ['nombreUsuario' => $userName, 'error' => '',  'userImage' => $userImage]);
        }
        else {
            return redirect('/');
        } 
    ...
    public function cambiarPasswordPost(Request $request){
        ...
        if($userPassword[0]->password == $request->oldPassword) {
            ...
            return view('administrativoUrgencias.cambiarPassword', ['nombreUsuario' => $userName, 'error' => 'Error: La contraseña actual es incorrecta.', 'userImage' => $userImage]);
        }
